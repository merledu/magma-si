...
class Muxes(implicit val config: MagmasiConfig) extends Module{
    ...
    when ((io.counterMatrix1(j)(i) =/= 0.U) && (io.counterMatrix2(k) =/= 0.U)) {
      when (srcValid(counter) === 0.B){
        when(io.counterMatrix1(j)(i) < io.counterMatrix2(k)){
          mux(counter) :=   (io.counterMatrix2(k) - 1.U) - (io.counterMatrix1(j)(i) - 1.U)
          src(counter) := io.mat2(k)
          srcValid(counter) := 1.B
          dest(counter) := io.mat1(j)(i)
        }.otherwise{
          mux(counter) :=   (io.counterMatrix1(j)(i) - 1.U) - (io.counterMatrix2(k) - 1.U)
          src(counter) := io.mat2(k)
          srcValid(counter) := 1.B
          dest(counter) := io.mat1(j)(i)
        }
