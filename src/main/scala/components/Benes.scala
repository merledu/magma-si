...
class Benes(implicit val config: MagmasiConfig) extends Module {
  val io = IO(new Bundle {
    ...
    val o_dist_bus  = Output(Vec(config.NUM_PES, UInt(config.DATA_TYPE.W)))
  ...
  val r_mux_bus_ff    = RegInit(VecInit(Seq.fill(2 * (config.LEVELS - 2) * config.NUM_PES + config.NUM_PES)(false.B)))
  val w_dist_bus      = Wire(Vec(config.NUM_PES, UInt(config.DATA_TYPE.W)))
  val w_internal      = Wire(Vec(2 * config.NUM_PES * (config.LEVELS - 1), UInt(config.DATA_TYPE.W))) // 8 x 2 = 16

  dontTouch(w_internal)

  r_data_bus_ff := Mux(rst, VecInit(Seq.fill(config.NUM_PES)(0.U(config.DATA_TYPE.W))), io.i_data_bus)
  r_mux_bus_ff  := Mux(rst, VecInit(Seq.fill(2 * (config.LEVELS - 2) * config.NUM_PES + config.NUM_PES)(false.B)), io.i_mux_bus)
  io.o_dist_bus := Mux(rst, VecInit(Seq.fill(config.NUM_PES)(0.U(config.DATA_TYPE.W))), w_dist_bus)

  for (i <- 0 until config.NUM_PES) {

    val in_switch = Module(new InputSwitch(config.DATA_TYPE))
    in_switch.io.in := r_data_bus_ff(i)
    w_internal(2 * i * (config.LEVELS - 1))      := in_switch.io.y
    w_internal(2 * i * (config.LEVELS - 1) + 1)  := in_switch.io.z
  ...
  for (i <- 0 until config.NUM_PES) { 
    ...
    out_switch.io.in0 := w_internal(2 * i * (config.LEVELS - 1) + (2 * (config.LEVELS - 2)))
    out_switch.io.in1 := w_internal(2 * (if(i % 2 == 0) i + 1 else i - 1) * (config.LEVELS - 1) + (2 * (config.LEVELS - 2)) + 1)
    out_switch.io.sel := r_mux_bus_ff(2 * config.NUM_PES * (config.LEVELS - 2) + i) 
    w_dist_bus(i)     := out_switch.io.y
  
  }

  for (i <- 0 until config.NUM_PES) {
    for (j <- 1 until (config.LEVELS - 1)) {
      val imm_switch = Module(new Switch(config.DATA_TYPE))
      imm_switch.io.in0 := w_internal(2 * i * (config.LEVELS - 1) + 2 * (j - 1))
      if (j <= (config.LEVELS - 1) / 2) {
        if (i % math.pow(2, j).toInt < math.pow(2, j - 1).toInt) {
          imm_switch.io.in1 := w_internal(2 * (i + math.pow(2, j - 1).toInt) * (config.LEVELS - 1) + 2 * (j - 1) + 1)
        } else {
          imm_switch.io.in1 := w_internal(2 * (i - math.pow(2, j - 1).toInt) * (config.LEVELS - 1) + 2 * (j - 1) + 1)
        }
      } else {
        if (i % math.pow(2, config.LEVELS - j).toInt < math.pow(2, config.LEVELS - j - 1).toInt) {
          imm_switch.io.in1 := w_internal(2 * (i + math.pow(2, config.LEVELS - j - 1).toInt) * (config.LEVELS - 1) + 2 * (j - 1) + 1)
        } else {
          imm_switch.io.in1 := w_internal(2 * (i - math.pow(2, config.LEVELS - j - 1).toInt) * (config.LEVELS - 1) + 2 * (j - 1) + 1)
        }
      }
      imm_switch.io.sel0 := r_mux_bus_ff(2 * (config.LEVELS - 2) * i + 2 * (j - 1))
      imm_switch.io.sel1 := r_mux_bus_ff(2 * (config.LEVELS - 2) * i + 2 * (j - 1) + 1)
      w_internal(2 * i * (config.LEVELS - 1) + 2 * j)      := imm_switch.io.y    
      w_internal(2 * i * (config.LEVELS - 1) + 2 * j + 1)  := imm_switch.io.z  
    }
  }
}
