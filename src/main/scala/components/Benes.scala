...
class Benes(implicit val config: MagmasiConfig) extends Module {
  val LEVELS   : Int = (2 * (math.log(config.NUM_IN) / math.log(2))).toInt + 1
  val io = IO(new Bundle {

    val i_data_bus2  = Input(Vec(config.NUM_IN, UInt(config.DATA_TYPE.W)))
    val i_data_bus1  = Input(Vec(config.NUM_IN, UInt(config.DATA_TYPE.W)))
    val i_mux_bus   = Input(Vec(2 * (LEVELS - 2) * config.NUM_IN + config.NUM_IN, Bool()))
    val o_dist_bus2  = Output(Vec(config.NUM_IN, UInt(config.DATA_TYPE.W)))
    val o_dist_bus1  = Output(Vec(config.NUM_IN, UInt(config.DATA_TYPE.W)))
  ...
  val r_data_bus_ff   = RegInit(VecInit(Seq.fill(config.NUM_IN)(0.U(config.DATA_TYPE.W))))
  val r_mux_bus_ff    = RegInit(VecInit(Seq.fill(2 * (LEVELS - 2) * config.NUM_IN + config.NUM_IN)(false.B)))
  val w_dist_bus      = Wire(Vec(config.NUM_IN, UInt(config.DATA_TYPE.W)))
  val w_internal      = Wire(Vec(2 * config.NUM_IN * (LEVELS - 1), UInt(config.DATA_TYPE.W))) // 8 x 2 = 16

  dontTouch(w_internal)

  r_data_bus_ff := Mux(rst, VecInit(Seq.fill(config.NUM_IN)(0.U(config.DATA_TYPE.W))), io.i_data_bus2)
  r_mux_bus_ff  := Mux(rst, VecInit(Seq.fill(2 * (LEVELS - 2) * config.NUM_IN + config.NUM_IN)(false.B)), io.i_mux_bus)
  io.o_dist_bus2 := Mux(rst, VecInit(Seq.fill(config.NUM_IN)(0.U(config.DATA_TYPE.W))), w_dist_bus)

  for (i <- 0 until config.NUM_IN) {
  ...
  for (i <- 0 until config.NUM_IN) { 
    ...
    out_switch.io.sel := r_mux_bus_ff(2 * config.NUM_IN * (LEVELS - 2) + i) 
