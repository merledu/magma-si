...
var nonZeroCount = 0
var noneachrow: Array[Int] = Array()
val rowcount = RegInit(VecInit(Seq.fill(4)(0.U(32.W))))
val i = RegInit(0.U(32.W))
val j = RegInit(0.U(32.W))
val mat = Reg(Vec(4, Vec(5,UInt(32.W))))
dontTouch(mat)
val count = Reg(Vec(4, UInt(32.W)))
dontTouch(count)
...
// Function to count non-zero elements in a vector
  def countNonZero(vector: Vec[UInt]): UInt = {
    vector.map(bit => Mux(bit =/= 0.U, 1.U, 0.U)).reduce(_ +& _)
  }

  // Count the number of non-zero elements in each row
  val rowCounters = io.Stationary_matrix.map(row => countNonZero(row))
