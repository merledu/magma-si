...
class ivncontrol4(implicit val Config: MagmasiConfig) extends Module {
 ...
 dontTouch(chec)
   mat(i)(j) := io.Stationary_matrix(i)(j)
 //   when(valid1=== false.B){
        when ( io.Stationary_matrix(i)(j) =/= 0.U && (j < (Config.MaxCols).U) && (io.in_valid)){
   ...
   // }
    ...
    dontTouch(j)
    dontTouch(rowcount)
    when ((i < (Config.MaxRows-1).U) && (j === (Config.MaxCols-1).U) && (io.in_valid)){
        i := i + 1.U
    }

    when ((j < (Config.MaxCols-1).U)&&(i <= (Config.MaxRows-1).U)  && (io.in_valid)){
        j := j + 1.U

    }.elsewhen(i === (Config.MaxRows-1).U && (j === (Config.MaxCols-1).U) && (io.in_valid)){
        j := j+1.U
    }.elsewhen(i === (Config.MaxRows-1).U && (j === (Config.MaxCols).U) && (io.in_valid)){
        j := j
