...
class FlexDPU(implicit val config:MagmasiConfig) extends Module{
    ...
    val high3 = RegInit(0.B)
    val delay = RegInit(0.B)

    when (io.valid && (iloop < (config.MaxRows-1).U) && (jloop < (config.MaxCols-1).U)){
        high3 := 1.B
  ...
  //val FDPE =
    ...
    val src1 = VecInit(io.Streaming_matrix(0)(0),io.Streaming_matrix(1)(0),0.U,0.U)
    val src2 = VecInit(io.Streaming_matrix(0)(1),io.Streaming_matrix(1)(1),0.U,0.U)
    dontTouch(src1)
    dontTouch(src2)
    val dest = VecInit(0.U,0.U,0.U,0.U)
            for (i <- 0 until config.NUM_PES){
            dest(i) := nonZeroValues(i)
        }
        //val PF1 = Module(new PathFinder)
        // val PF = VecInit(Seq.fill(1)(Module(new PathFinder).io))

        // for (i <- 0 until 1){
        // PF(0).Ivalid := RegNext(Statvalid)
        ...
        val PF = Module(new PathFInder2)
        PF.io.src := src1
        PF.io.dest := dest
        PF.io.valid := Statvalid
        PF.io.StaMat := io.Stationary_matrix
        PF.io.StrMat :=           
...
//             for (j <- 0 until 4){
//                     FDPE(i).i_vn(j) := 1.U
//                         FDPE(i).i_mux_bus(j) := Reverse(PF(0).i_mux_bus(j))
//             }
...
//         //     cc := cc
//         // }
//         val delay4 = RegInit(0.U(16.W))
//         val high4 = RegInit(0.B)

//         when(PF(0).PF_Valid){
//             high4 := 1.B
//             delay4 := delay4 + 1.U
//         }.elsewhen(high4&&(delay4 > 0.U)){
//             delay4 := delay4 + 1.U
//             high4 := 1.B
//         }

//         when (ModuleIndex === (config.MaxRows -1).U ){
//             ModuleIndex := ModuleIndex
//         }

//         when (high4 && (delay4 === 20.U) && (ModuleIndex < (config.MaxRows -1).U )){
//             high4 := 0.B
//             delay4 := 0.U
